...
def gru_layer(tparams, state_below, options, prefix='gru', mask=None,
    ...
    def _step_slice(m_, x_, xx_, h_, U, Ux):
        # reset and update gates, with combination
        ...
        cand_h = tensor.tanh(tensor.dot(h_, Ux) * r + xx_)
...
def gru_cond_layer(tparams, state_below, options, prefix='gru',
    ...
    def _step_slice(m_, x_, xx_, h_, ctx_, alpha_, pctx_, cc_,
        ...
        preact1 = tensor.nnet.sigmoid(tensor.dot(h_, U) + x_)
        r1 = _slice(preact1, 0, dim)
        u1 = _slice(preact1, 1, dim)

        cand_x1 = tensor.tanh(tensor.dot(h_, Ux) * r1 + xx_)

        h1 = u1 * h_ + (1. - u1) * cand_x1
        ...
        preact2 = tensor.nnet.sigmoid(tensor.dot(h1, U_nl) + tensor.dot(ctx_, Wc) + b_nl)

        r2 = _slice(preact2, 0, dim)
        u2 = _slice(preact2, 1, dim)

        cand_x2 = tensor.tanh((tensor.dot(h1, Ux_nl) + bx_nl ) * r2 + tensor.dot(ctx_, Wcx))

        h2 = u2 * h1 + (1. - u2) * cand_x2
